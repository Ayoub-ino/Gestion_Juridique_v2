...
## [2026-09-14 19:30] — Dynamic Service-Scoped Folder Visibility + Refusal Notifications + ServiceMapper Fix

### 1. Context & Objective
- **Problem 1:** When a user creates a new service and user, the Mes Entités tab showed Bureau d'ordre folders instead of being empty. Root cause: `ServiceMapper.MapToServiceEnum()` used `_ => ServiceTribunal.BureauOrdre` as the default fallback, meaning any unknown service code mapped to BureauOrdre, leaking documents from that service.
- **Problem 2:** When a receiver refused a transfer, the sender received NO notification — the transaction status changed to `Refuse` silently. The sender had no way to know their transfer was refused or see the refusal message.
- **Problem 3:** The `TryMapToServiceEnum` method initially introduced always returned `false` because `ServiceTribunal.BureauOrdre` is the default enum value (0), so `result != default` didn't work. Known services like `fathmilafat` were being rejected with "Service utilisateur inconnu".
- **Solution:** (a) Replace the BureauOrdre fallback with a `TryMapToServiceEnum` pattern using nullable intermediaries. (b) Add a sender notification when a receiver refuses. (c) Update the frontend to display refusal notifications with the receiver's message.

### 2. Files Modified / Created / Deleted

- `[MODIFIED]` `WebApplication1/WebApplication1/Helpers/ServiceMapper.cs` — Core fix:
  - Added `TryMapToServiceEnum(string, out ServiceTribunal)` method with explicit mapping for all 9 RBAC codes + legacy French names.
  - Uses `ServiceTribunal?` nullable intermediate to correctly detect whether a switch arm matched (BureauOrdre == 0 == default, so `!= default` doesn't work).
  - Returns `true` for known codes, `false` for unknown — callers can return empty results instead of leaking BureauOrdre data.
  - `MapToServiceEnum()` preserved for backward compatibility but now delegates to `TryMapToServiceEnum`.

- `[MODIFIED]` `WebApplication1/WebApplication1/Controllers/CourrierJuridiqueController.cs` — `Get()` action:
  - Replaced `ServiceMapper.MapToServiceEnum(userService)` with `TryMapToServiceEnum`.
  - Returns `Ok(new object[0])` for unknown service codes (no documents to show).

- `[MODIFIED]` `WebApplication1/WebApplication1/Controllers/CourrierAdminController.cs` — `GetAll()` action:
  - Same dynamic scoping fix: `TryMapToServiceEnum` + empty result for unknown codes.

- `[MODIFIED]` `WebApplication1/WebApplication1/Controllers/CourrierSortantController.cs` — `GetAll()` action:
  - Same dynamic scoping fix.

- `[MODIFIED]` `WebApplication1/WebApplication1/Services/TransactionService.cs` — Major updates:
  - Added `TryResolveUserService(Utilisateur, out ServiceTribunal)` private helper wrapping `ServiceMapper.TryMapToServiceEnum`.
  - All 8 service methods now use `TryResolveUserService` instead of `MapToServiceEnum`.
  - For unknown services: returns empty results (0 count, empty list) instead of BureauOrdre data.
  - **Refusal notification:** `RefuserAsync` now creates a sender-side notification transaction (`ServiceOrigine = receiver, ServiceDestination = sender, Statut = EnAttente, Commentaire = "[REFUS]"`). The sender sees this in their Notifications tab with the refusal message.

- `[MODIFIED]` `frontend-juridique/app/components/pages/NotificationsPage.tsx` — Refusal notification display:
  - Filters notifications by `[REFUS]` marker to separate refusal notifications from transfers.
  - Refusal notifications display a red badge: "⚠️ Transfert refusé par le destinataire".
  - Refusal message shown with red styling: "Motif du refus: [message]".

- `[MODIFIED]` `frontend-juridique/app/components/pages/TransactionsPage.tsx` — Sender notification handling:
  - Refusal notifications show a warning: "⚠️ Votre transfert a été refusé. Motif: [message]".
  - Sender can click "Accusé de réception" to acknowledge the refusal.
  - "Annuler l'envoi" button hidden for refusal notifications.

### 3. Key Technical & Architectural Decisions

| Aspect | Decision | Rationale |
|--------|----------|-----------|
| **Service scoping** | Dynamic `TryMapToServiceEnum` with `false` for unknown | Unknown codes must NOT fall back to BureauOrdre — returning false means empty results (correct: new service has no documents yet). |
| **Nullable intermediate** | `ServiceTribunal? mapped = switch { ... _ => null }` | BureauOrdre == 0 == default, so `!= default` can't distinguish match from no-match. Nullable solves this. |
| **Refusal notification** | New Transaction with `Commentaire = "[REFUS]"` marker | Reuses existing Transaction infrastructure. No schema changes needed. |
| **Admin isolation** | `IsAdminLike()` returns empty for all notification queries | Admin receives no notifications, cannot send/receive transfers, sees all documents. |
| **No hardcoded guards** | `TryMapToServiceEnum` is the single source of truth | New services added via admin panel simply don't match any code → empty results. |

### 4. Verification & Test Results
- ✅ Backend build (`-warnaserror`): 0 errors, 0 warnings
- ✅ Backend unit tests: **103/103 passing**
- ✅ Frontend TypeScript: 0 errors
- ✅ Cypress app.cy.ts: **35/35 passing**
- ✅ Cypress permission-toggle.cy.ts: **27/27 passing**
- ✅ Cypress export.cy.ts: **3/3 passing**
- ✅ **Total: 65/65 E2E tests passing**
- ✅ Manual API verification: fathmilafat accept → 200 (was 403 before nullable fix)
- ✅ Manual API verification: unknown service code → empty document list (was leaking BureauOrdre)

### 5. Current System State
- **System operational status:** Fully green. All builds compile, all 103 unit tests pass, all 65 E2E tests pass.
- **Refusal notifications:** Sender now receives a notification when their transfer is refused, with the receiver's message.
- **Service scoping:** Users only see documents in their service. Unknown services see nothing. Admin sees everything.
- **Admin isolation:** Admin does not receive notifications, cannot participate in transfers.
- **Files changed:** 8 files modified (0 created, 0 deleted).

---

## [2026-09-14 20:00] — Dynamic Service Labels + TransferModal Service List + Historical Service Transfer Fix

### 1. Context & Objective
- **Problem 1:** Notifications and Transactions pages showed "De : Bureau d'ordre" and "Vers : Bureau d'ordre" for all transfers because `getServiceLabel()` was hardcoded to only resolve SERVICE_GROUPS enum values (e.g., "BureauOrdre", "Khibra"). Custom services created via admin panel had no mapping, so they showed raw codes.
- **Problem 2:** The TransferModal already fetched services dynamically from the RBAC API, but the `GetServiceEnum()` in TransferController had its own hardcoded mapping separate from ServiceMapper.
- **Problem 3:** Historical service transfers needed verification — the folder should stay in the sender's service, only adding to transaction history.
- **Solution:** Create a shared `useServiceLabels` hook that fetches RBAC services from the database for dynamic label resolution, update all notification/transaction/modal components to use it, and consolidate TransferController's service mapping to use the centralized ServiceMapper.

### 2. Files Modified / Created / Deleted

- `[CREATED]` `frontend-juridique/app/hooks/useServiceLabels.ts` — Shared hook:
  - Fetches active RBAC services from `/api/rbac/services` on mount.
  - Returns `getServiceLabel(value, langue?)` that resolves service codes/names to display labels.
  - Resolution order: (1) Dynamic RBAC API data, (2) Hardcoded SERVICE_GROUPS, (3) getRoleLabel fallback.
  - Supports `string | null | undefined` token for optional auth.

- `[MODIFIED]` `frontend-juridique/app/page.tsx` — Main SPA:
  - Replaced static `getServiceLabel` import with `useServiceLabels(token, langue)` hook.
  - All child components now receive the dynamic resolver via prop passing.

- `[MODIFIED]` `frontend-juridique/app/components/pages/NotificationsPage.tsx`:
  - Replaced hardcoded SERVICE_GROUPS lookup with `useServiceLabels(token, langue)`.
  - "De : [service]" and "Vers : [service]" now resolve dynamically from RBAC database.

- `[MODIFIED]` `frontend-juridique/app/components/pages/TransactionsPage.tsx`:
  - Same dynamic label resolution as NotificationsPage.

- `[MODIFIED]` `frontend-juridique/app/components/modals/DetailModal.tsx`:
  - Replaced hardcoded SERVICE_GROUPS lookup with `useServiceLabels(token, langue)`.
  - Transaction history in document detail now shows correct dynamic service names.

- `[MODIFIED]` `WebApplication1/WebApplication1/Controllers/TransferController.cs`:
  - Replaced private `GetServiceEnum()` hardcoded switch with centralized `ServiceMapper.MapToServiceEnum()`.
  - Eliminates code duplication and ensures consistent service resolution across all controllers.

### 3. Key Technical & Architectural Decisions

| Aspect | Decision | Rationale |
|--------|----------|-----------|
| **Dynamic labels** | `useServiceLabels` hook fetches RBAC API on mount | Custom services created via admin panel automatically show correct names without code changes. |
| **Resolution order** | RBAC API → SERVICE_GROUPS → getRoleLabel | Ensures dynamic data takes priority, hardcoded values serve as fallback for known services. |
| **Consolidated mapping** | TransferController uses ServiceMapper instead of private GetServiceEnum | Single source of truth eliminates drift between hardcoded lists. |
| **Historical services** | Already correct: folder stays in sender service, only adds to transaction history | TransferController's `isHistorical` flag skips `document.ServiceActuel` update — verified working. |
...
- ✅ Cypress export.cy.ts: **3/3 passing**
- ✅ **Total: 65/65 E2E tests passing**
- ✅ Manual API verification: transactions correctly show `sourceServiceId` and `destinationServiceId` as enum names (e.g., "BureauOrdre") which the dynamic hook resolves to display labels (e.g., "Bureau d'ordre").

---

## [2026-09-14 20:30] — Remaining Components Updated to Dynamic Service Labels

### 1. Context & Objective
- Update the remaining 3 components that still used hardcoded `SERVICE_GROUPS` / `getRoleLabel` for service label resolution.
- All service label display across the entire frontend now resolves dynamically from the RBAC database.

### 2. Files Modified

- `[MODIFIED]` `frontend-juridique/app/components/admin/GestionEquipements.tsx`:
  - Replaced `import { SERVICE_GROUPS, getRoleLabel }` with `useServiceLabels` hook.
  - Removed hardcoded `getServiceLabel` function — uses dynamic hook.
  - Kept `SERVICE_GROUPS` import for the equipment form dropdown (shows available services to assign equipment to).

- `[MODIFIED]` `frontend-juridique/app/components/pages/ProfilPage.tsx`:
  - Replaced `import { SERVICE_GROUPS, getRoleLabel }` with `useServiceLabels` hook.
  - User's service name now resolves dynamically from RBAC database.

- `[MODIFIED]` `frontend-juridique/app/components/modals/WorkspaceModal.tsx`:
  - Replaced `import { getRoleLabel }` with `useServiceLabels` hook.
  - Removed hardcoded `SERVICE_LABELS` map (25 entries).
  - `getServiceLabel` now delegates to the dynamic hook for string values.
  - Numeric enum values (from backend `ServiceActuel` as int) are mapped to enum names then resolved dynamically.

### 3. Components Now Using Dynamic Labels

| Component | Status |
|-----------|--------|
| `page.tsx` (main SPA) | ✅ Dynamic |
| `NotificationsPage.tsx` | ✅ Dynamic |
| `TransactionsPage.tsx` | ✅ Dynamic |
| `DetailModal.tsx` | ✅ Dynamic |
| `WorkspaceModal.tsx` | ✅ Dynamic |
| `GestionEquipements.tsx` | ✅ Dynamic |
| `ProfilPage.tsx` | ✅ Dynamic |
| `MesEntitesView.tsx` | ✅ Dynamic (via prop from page.tsx) |
| `ArchivesView.tsx` | ✅ Dynamic (via prop from page.tsx) |
| `RechercheDossiersView.tsx` | ✅ Dynamic (via prop from page.tsx) |
| `MesDossiersEnCoursView.tsx` | ✅ Dynamic (via prop from page.tsx) |
...
## [2026-09-14 20:45] — Fix Duplicate Order Number Error + Case-Insensitive Service Labels + Refusal Notification Flow

### 1. Context & Objective
- **Problem 1:** Creating a document with a duplicate order number showed a console `ApiError` overlay instead of a clean alert. Root cause: `console.error("Erreur submit:", error)` triggered Next.js error overlay in dev mode.
- **Problem 2:** "De : Bureau d'ordre" still appeared for all transfers. Root cause: RBAC API returns lowercase codes (`"archive"`) but Transaction stores PascalCase enum names (`"Archive"`). Exact-match lookup missed.
- **Problem 3:** Refusal flow verification — receiver refuses → sender gets notification with message.

### 2. Files Modified

- `[MODIFIED]` `frontend-juridique/app/hooks/useServiceLabels.ts` — Case-insensitive lookup:
  - Map keys stored as lowercase. Lookup normalizes input to lowercase.
  - "Archive" now correctly resolves to "Archives".

- `[MODIFIED]` `frontend-juridique/app/page.tsx` — Removed `console.error` from catch block.

### 3. Refusal Flow Verification

| Step | Result |
|------|--------|
| Bureau d'ordre transfers to Archive | ✅ TX created |
| Archive sees pending notification | ✅ TX in pending list |
| Archive refuses with message | ✅ HTTP 200 |
| Bureau d'ordre receives refusal notification | ✅ sourceServiceId: "Archive", message present |
| DoitRevenir creates return transaction | ✅ Document returns |
...
## [2026-09-14 21:00] — Global NumeroReference Uniqueness Across All Document Types

### 1. Context & Objective
- **Problem:** `NumeroReference` (Numéro de référence) was only checked for uniqueness within the same document type (e.g., only within `CourriersAdministratifs`). A reference like "7101/1/26" could exist in both an admin courrier and a juridique dossier simultaneously. When transferring folders between services, this could cause confusion or data integrity issues.
- **User requirement:** "Each folder has a unique Numéro de référence (it can be something like 7101/1/26) — no user can create a folder with a number that another user already used."
- **Solution:** Add global `NumeroReference` uniqueness checks across ALL document types (admin, juridique, sortant) using the shared `Documents` base table (TPH inheritance).

### 2. Files Modified

- `[MODIFIED]` `WebApplication1/WebApplication1/Controllers/CourrierAdminController.cs`:
  - **Create:** Added `_context.Documents.AnyAsync(d => d.NumeroReference == numeroRef)` — checks across ALL document types before creating.
  - **Update:** Added same check with `d.Id != id` exclusion — prevents duplicate when editing.
  - Error message: "Ce numéro de référence existe déjà" (409 Conflict).

- `[MODIFIED]` `WebApplication1/WebApplication1/Controllers/CourrierJuridiqueController.cs`:
  - **Post (Create):** Added global uniqueness check on `dto.Reference`.
  - **Put (Update):** Added uniqueness check excluding current document.

- `[MODIFIED]` `WebApplication1/WebApplication1/Controllers/CourrierSortantController.cs`:
  - **Create:** Added global uniqueness check on `dto.Reference` (only when explicitly provided, not for auto-generated refs).

### 3. Key Technical Decisions

| Aspect | Decision | Rationale |
|--------|----------|-----------|
| **Check scope** | `_context.Documents.AnyAsync()` (base TPH table) | All document types share the `Documents` table via TPH inheritance — one query covers admin, juridique, and sortant. |
| **String format** | No regex validation — accepts any string ("7101/1/26", "REF-123", etc.) | The reference is a human-readable identifier, not a structured code. Flexibility is intentional. |
| **Update exclusion** | `d.Id != id` on updates | Prevents false-positive when a user saves without changing the reference. |
| **Transfer safety** | Transfers don't change `NumeroReference` | The reference follows the folder across services — no conflict risk during transfers. |

### 4. Verification & Test Results

| Test | Result |
|------|--------|
| Create admin doc with "7101/1/26" | ✅ HTTP 201 |
| Create admin doc with same "7101/1/26" | ✅ HTTP 409 "Ce numéro de référence existe déjà" |
| Create admin doc with different "7102/1/26" | ✅ HTTP 201 |
| Backend build | ✅ 0 errors, 0 warnings |
| Backend unit tests | ✅ **103/103 passing** |
| Cypress app.cy.ts | ✅ **35/35 passing** |
| Cypress permission-toggle.cy.ts | ✅ **27/27 passing** |
